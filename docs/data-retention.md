...
| User-local project `telemetry/attempts.jsonl`, `runs.jsonl`, `summary.json` | Operator telemetry outside the repository worktree | Keep run rows indefinitely; rotate old raw attempt rows only after preserving their monthly aggregate. `summary.json` is an atomic, lock-protected projection and may be replaced at any time. | `orboros telemetry rebuild --project <name>` recreates attempts and `summary.json` from that project's `executions.jsonl`; open started rows remain visible as active until rebuilt or completed. |
...
| Project `logs/executions.jsonl` and `logs/heddle/` | Keep one durable outer-attempt record and Heddle diagnostics together for each registered project | Retain execution records; prune raw Heddle state/transcripts only after preserving the corresponding structured attempt records. Rotate `daemon.log` and `cli.log` together with their diagnostic window. | Locate a record by orb ID, worker ID, or `attempt-<n>` under `~/.orboros/projects/<project-key>/logs/`; benchmark artifacts remain in their isolated run. |
